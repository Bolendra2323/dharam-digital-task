...
const createCampaign = async function(req, res) {
    try {
        ...
        let campaignAlreadyExist = await campaigncollectionModel.findOne({ name: data.name, offers: data.offers });
        ...
        if (campaignAlreadyExist)
            ...
            return res.status(400).send({ status: false, message: "name and offers already present" });

        const createCampaign = await campaigncollectionModel.create(data);

        //line no.34 is used for creating the Campaign data

        let { name, offers } = createCampaign;

        return res.status(201).send({ status: true, data: { name, offers } });
